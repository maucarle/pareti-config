...
function demoProject(){
  const f1={id:uid('f'),name:'30×45',w:30,h:45,pp:2,cw:1.5,cc:'nera',central:false};
  const f2={id:uid('f'),name:'20×30',w:20,h:30,pp:2,cw:1.5,cc:'nera',central:false};
  const f3={id:uid('f'),name:'13×19,5',w:13,h:19.5,pp:2,cw:1.5,cc:'nera',central:false};
  return {
...
function migrateStore(){
  store.projects.forEach(p=>{
    (p.formats||[]).forEach(f=>{ if(f.cw==null)f.cw=1.5; if(!f.cc)f.cc='nera'; if(f.central==null)f.central=false; });
...
function makeFrameEl(p, s, showLabel, num){
  const color=fmtColor(p.fmt.id);
  const ppPx=Math.max(1,p.fmt.pp*s);
  const cw=p.fmt.cw||0;
  const fw= cw<=0 ? 0 : Math.max(1.2, cw*s);
  const isWhite=p.fmt.cc==='bianca';
  ...
  el.className='cfrm';
  el.title=`${p.fmt.name}${p.rot?' (orizzontale)':' (verticale)'} — da sinistra ${p.x.toFixed(0)} cm · dall'alto ${p.y.toFixed(0)} cm`;
  el.style.cssText=
    `position:absolute;left:${(p.x*s).toFixed(1)}px;top:${(p.y*s).toFixed(1)}px;width:${(p.w*s).toFixed(1)}px;height:${(p.h*s).toFixed(1)}px;z-index:2;`+
    `background:${frameBg};padding:${fw}px;`+
    `box-shadow:${frameRing};`;
  ...
  const innerContent=photoSrc
    ...
    : (showLabel?`<span style="font-family:var(--sans);font-size:6px;color:rgba(40,30,18,.45);letter-spacing:.3px;">${p.fmt.name}</span>`:'');
  el.innerHTML=
    `<div style="width:100%;height:100%;background:var(--mat);padding:${ppPx.toFixed(1)}px;box-shadow:inset 0 1px 3px rgba(60,45,25,.28),inset 0 0 0 .5px rgba(0,0,0,.12);">`+
      `<div style="width:100%;height:100%;background:${innerBg};box-shadow:inset 0 0 0 .5px rgba(0,0,0,.18),inset 0 1px 4px rgba(0,0,0,.14);display:flex;align-items:center;justify-content:center;overflow:hidden;">`+
        innerContent+
      `</div></div>`;
  if(num!=null){
...
function renderLegend(){
  ...
  P.formats.forEach(f=>{
    ...
    sp.innerHTML=`<i style="background:${fmtColor(f.id)};"></i> ${f.name} cm <small>(cornice ${fmtNum(f.cw||0)} cm ${f.cc==='bianca'?'bianca':'nera'} · pp ${f.pp})${f.central?' · <b style="color:var(--brass)">centrale</b>':''}</small>`;
    lg.appendChild(sp);
  });
...
function renderSettings(){
  ...
  fh.innerHTML='<span style="width:90px;">Nome</span><span style="width:54px;">Foto L</span><span style="width:54px;">Foto H</span><span style="width:50px;">Passep.</span><span style="width:50px;">Cornice</span><span style="width:62px;">Colore</span><span style="width:46px;">Centro</span>';
  ft.appendChild(fh);
  P.formats.forEach(f=>{
    const row=document.createElement('div'); row.className='ed-row';
    row.appendChild(textInput(f.name,v=>{ f.name=v||(f.w+'×'+f.h); refresh(); renderSettings(); },90));
    row.appendChild(numInput(f.w,5,200,0.5,v=>{ f.w=v; refresh(); },54));
    row.appendChild(numInput(f.h,5,200,0.5,v=>{ f.h=v; refresh(); },54));
    row.appendChild(numInput(f.pp,0,15,0.5,v=>{ f.pp=v; refresh(); },50));
    row.appendChild(numInput(f.cw==null?1.5:f.cw,0,15,0.5,v=>{ f.cw=v; refresh(); },50));
    row.appendChild(selectInput(f.cc||'nera',[{v:'nera',t:'Nera'},{v:'bianca',t:'Bianca'}],v=>{ f.cc=v; refresh(); },62));
  ...
  addF.onclick=()=>{ P.formats.push({id:uid('f'),name:'50×70',w:50,h:70,pp:3,cw:1.5,cc:'nera',central:false}); renderSettings(); refresh(); };
  ft.appendChild(addF);
